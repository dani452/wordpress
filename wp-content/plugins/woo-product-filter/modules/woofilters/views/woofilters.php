...
class WoofiltersViewWpf extends ViewWpf {
	...
	public function getEditTabContent( $idIn ) {
		...
		FrameWpf::_()->addScript('admin.wp.colorpicker.alhpa.js', $modPath . 'js/admin.wp.colorpicker.alpha.js', array('wp-color-picker'));
			...
			if (has_shortcode( $obj->post_content, 'products' )) {
				...
				if (!empty($matches)) {
					...
					if (isset($attr['category'])) {
						...
						if (intval($category_name[0])) {
							$cat = get_term_by('id', intval($category_name[0]), 'product_cat');
						} else {
	...
	public function generateFiltersHtml( $filterSettings, $viewId, $prodCatId = false, $noWooPage = false, $taxonomies = array() ) {
		...
		$html = '<div class="wpfMainWrapper" id="' . $filterId . '" data-viewid="' . $viewId . '" data-filter="' . explode('_', $viewId)[0] . '" data-settings="' . $querySettingsStr . '" data-filter-settings="' . $filterSettings . '" ' . $noWooPageData . '>';
	...
	public function generateAttributeFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$html =
			'<div id="' . self::$blockId .
				...
				'" data-slug="' . esc_attr__($filterNameSlug, 'woo-product-filter') .
